...
public class RaceManager {
    ...
    private Race parseRace(String id, ConfigurationSection rs) {
        ...
        return new Race(id.toLowerCase(), displayName, description, icon,
                lpGroup, maxHealth, speed, damage, knockback, passives, conditions);
    }
    ...
    private Condition parseCondition(Map<?, ?> map) {
        ...
        if (triggerName == null || effectName == null) return null;

        try {
            Condition.Trigger trigger = Condition.Trigger.valueOf(triggerName.toUpperCase());
            PassiveAbility.Type effectType = PassiveAbility.Type.valueOf(effectName.toUpperCase());
            int amp = map.containsKey("amp") ? ((Number) map.get("amp")).intValue() : 0;
            PassiveAbility effect = new PassiveAbility(effectType, amp);
            ...
            return new Condition(trigger, yValue, biomes, world, effect);
        } catch (IllegalArgumentException e) {
            plugin.getLogger().warning("Invalid condition trigger/effect: " + triggerName + " / " + effectName);
            return null;
        }
    }
